...
import { Eraser, Pencil, RefreshCw, FileImage, Plus, Grid3X3, AlignJustify, Square, Palette, Circle, Hand } from 'lucide-react';
...
export const HandwritingCanvas = forwardRef<HandwritingCanvasRef, HandwritingCanvasProps>(({ 
  ...
  const debouncedSaveRef = useRef<((data: string) => void) | null>(null);
  const makeDebouncedSave = (handler?: (data: string) => void) => {
  ...
  useEffect(() => {
    debouncedSaveRef.current = makeDebouncedSave(onSave);
  }, [onSave]);
  ...
  useEffect(() => {
    ...
    const animId: number = requestAnimationFrame(() => {
        redraw();
    });
  ...
  useEffect(() => {
    ...
    const getAvgY = (touches: TouchList) => {
      let sum = 0;
      for (let i = 0; i < touches.length; i++) sum += touches.item(i)!.clientY;
  ...
  const handleClear = () => {
    setLines([]);
    debouncedSaveRef.current?.(JSON.stringify({ lines: [], width: dimensions.width, height: dimensions.height }));
